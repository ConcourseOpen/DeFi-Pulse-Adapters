...
async function getMarkets(block) {
  ...
  let [underlyingDecimals, underlyingSymbols] = await Promise.all([
  ...
  _.each(allUnderlying, (underlying) => {
    let decimals = _.find(
      ...
      (decimals) => decimals.success && decimals.input.target === underlying.output
    );
    let symbol = _.find(
...
async function tvl(timestamp, block) {
  ...
  let [allUnderlying, cashes, v2Balances, yETHPirce, yUSDPrice, yCrvPrice, creth2Reserve] = await Promise.all([
...
async function rates(timestamp, block) {
  ...
  _.each(markets, (data, underlying) => {
    let supplyRate = _.find(supplyResults, (result) => result.success && result.input.target == data.cToken);
    let borrowRate = _.find(borrowResults, (result) => result.success && result.input.target == data.cToken);
    let totalBorrows = _.find(totalBorrowsResults, (result) => result.success && result.input.target == data.cToken);

    if (supplyRate && borrowRate && totalBorrows) {
      let symbol = data.symbol;
      rates.lend[symbol] = String(((1 + supplyRate.output / 1e18) ** (365 * 5760) - 1) * 100);
      rates.borrow[symbol] = String(((1 + borrowRate.output / 1e18) ** (365 * 5760) - 1) * 100);
