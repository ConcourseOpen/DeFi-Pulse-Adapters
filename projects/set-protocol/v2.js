  ...
  _.each(positionsForSets, function (positionForSet, i) {
    ...
    _.each(positionForSet.output, (position) => {
      ...
      if (isExternalPosition) {
        uniswapPositions[componentAddress] = BigNumber(uniswapPositions[componentAddress] || 0)
  ...
  _.each(reserves, function (reserve, i) {
    ...
    if (tokenPair[0]) {
      ...
      if (!reserve0.isZero()) {
        ...
        balances[tokenPair[0]] = existingBalance.plus(reserve0.times(setSupplyRatio)).toFixed();
      }
    }
    ...
    if (tokenPair[1]) {
      ...
      if (!reserve1.isZero()) {
        ...
        balances[tokenPair[1]] = existingBalance.plus(reserve1.times(setSupplyRatio)).toFixed();
      }
    }
  });
