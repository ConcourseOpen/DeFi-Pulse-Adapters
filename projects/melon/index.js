...
/*==================================================
  ...
  async function run(timestamp, block) {
    ...
    let latestIds = (await sdk.api.abi.multiCall({
      block,
    ...
    let funds = (await sdk.api.abi.multiCall({
      block,
      calls: fundCalls,
      abi: {
        "constant":true,
    ...
    let accountingCalls = _.map(_.uniq(_.pluck(_.filter(funds, (fund) => { return fund.success}), 'output')), (fund) => {
      return {
        target: fund
      }
    });

    let accounting = (await sdk.api.abi.multiCall({
      block,
    ...
    let fundHoldings = (await sdk.api.abi.multiCall({
      block,
      calls: holdingCalls,
      abi: {
        "constant": false,
    ...
    _.each(fundHoldings, (holdings) => {
      if(holdings.success) {
        ...
        _.each(output, (balance) => {
          ...
          let value = balance[1];
          balances[address] = BigNumber(balances[address] || 0).plus(value).toFixed();
        });
      }
    });
