...
async function tvl(timestamp, block) {
  ...
  if (block < 9518500) {
    ...
    let dharmaDaiUnderlyingBalance = (
    ...
    let dharmaDaiUnderlyingBalanceBN = BigNumber(dharmaDaiUnderlyingBalance || 0);
    if (dharmaDaiUnderlyingBalanceBN.toNumber() > 0) {
      balances[daiAddress] = dharmaDaiUnderlyingBalanceBN.toFixed();
    }

    let dharmaUSDCUnderlyingBalance = (
      await sdk.api.abi.call({
        block,
    ...
    let dharmaUSDCUnderlyingBalanceBN = BigNumber(dharmaUSDCUnderlyingBalance || 0);
