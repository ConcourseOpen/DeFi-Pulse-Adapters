...
async function _multiMarketV1Tvl(lendingPoolCore, reserves, block) {
  let balances = {
    "0x0000000000000000000000000000000000000000": (await sdk.api.eth.getBalance({ target: lendingPoolCore, block }))
      .output,
  };
...
async function getV2Tvl(block) {
  const underlyingAddressesDict = Object.keys(v2ReserveTokens).map((key) => v2ReserveTokens[key]);
...
async function tvl(timestamp, block) {
  ...
  const uniswapMarketTvlBalances = await _multiMarketV1Tvl(uniswapLendingPoolCore, uniswapReserves, block);

  // ...add v1 uniswap market TVL
  Object.keys(uniswapMarketTvlBalances).forEach((address) => {
    if (balances[address]) {
      balances[address] = BigNumber(balances[address]).plus(uniswapMarketTvlBalances[address]).toFixed();
    } else {
  ...
  if (block >= 11360925) {
    ...
    v2Tvl.map((data) => {
      if (balances[data.underlying]) {
        balances[data.underlying] = BigNumber(balances[data.underlying]).plus(data.balance).toFixed();
      } else {
