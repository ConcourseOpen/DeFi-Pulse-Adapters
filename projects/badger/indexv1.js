...
async function tvl(timestamp, block) {
  ...
  _.each(geyserBalances.output, (geyserBalance) => {
    if (geyserBalance.success) {
      ...
      balances[underlyingTokenAddress[geyserAddress]] = BigNumber(valueInGeyser).multipliedBy(
        settPpfs[underlyingTokenAddress[geyserAddress]]
      );
    }
  });
  ...
  _.each(settBalances.output, (settBalance) => {
    if (settBalance.success) {
      ...
      balances[underlyingTokenAddress[settAddress]] = BigNumber(balances[underlyingTokenAddress[settAddress]]).plus(
        valueInSett
      );
    }
  });
