...
function getPmCalls(timestamp) {
  ...
  for (let i = 0; i < allPortfolioManagers.length; i++) {
    if (
      ...
      (allPortfolioManagers[i].endTimestamp == null || timestamp <= allPortfolioManagers[i].endTimestamp)
    ) {
      let calls = _.reduce(
        tokenAddresses,
        (accum, tokenAddress) => [
...
function getOpportunityCalls(timestamp) {
  ...
  for (let i = 0; i < allOpportunities.length; i++) {
    if (
      ...
      (allOpportunities[i].endTimestamp == null || timestamp <= allOpportunities[i].endTimestamp)
    ) {
